...
namespace ProtoLib.Managers
{
    public class AnalyticManager
    {
        public object PostStatus()
        {
            ...
            { 
                var posts = c.Posts.AsNoTracking().ToList();
                ...
                foreach (var post in posts)
                {
                    ...
                    var actualWorks = c.Works.AsNoTracking().
                        Include(x=>x.Issues).Where(x =>  x.Status != WorkStatus.ended && x.PostId == post.Name)
                        .ToList();
                    ...
                    foreach (var status in statusList)
                    {
                        ...
                        statusStat.Issues = statusWorks.SelectMany(x => x.Issues).Where(x=>x.Resolved==null).Count();
        ...
        public object OrderStat(long orderId)
        {
            ...
            using (BaseContext c = new BaseContext(""))
            {
                var posts = c.Posts.ToList();
                var orderWorks = c.Works.AsNoTracking().Include(x=>x.Issues).Where(x => x.OrderNumber == orderId).ToList();
                ...
                result.IssuesText = String.Join("\r\n",orderWorks.SelectMany(x => x.Issues).Where(x => x.Resolved == null)
                ...
                foreach (var article in articleGroup)
                {
                    ...
                    foreach (var post in postGroup)
                    {
                        ...
                        var works = post.Where(x => 
                                                    x.PrevStatus != WorkStatus.unkown); //чтобы отсечь прогнозируемые работы но при это сохранить все артикула
